...
export default function LoginPage() {
    ...
    return (
        <div className="bg-cc-bg min-h-screen flex font-dm border-box items-center justify-center border-[16px] border-cc-outer">
                    ...
                    >
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                <p className="text-cc-muted font-bold text-sm mb-8">
                                    {tab === 'login'
                                        ? 'Sign in to access your tech roadmap.'
                                        : "It's free. No credit card required."}
                                </p>

                                <button
                                    type="button"
                                    className="w-full flex items-center justify-center gap-3 py-3 rounded-xl bg-white border-2 border-cc-border text-cc-text font-bold hover:bg-cc-gray transition-colors mb-6 shadow-neo disabled:opacity-50"
                                    onClick={() => {
                                        setError('Google auth requires API keys. Use Demo Login below to view UI.');
                                    }}
                                >
                                    <GoogleIcon />
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
                                    ...
                                    {tab === 'signup' && (
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-white border-2 border-cc-border rounded-xl py-3 pl-12 pr-4 text-cc-text font-bold placeholder:text-cc-muted focus:outline-none focus:bg-cc-yellow/10 focus:shadow-neo transition-all"
                                                required
                                            />
                                        </div>
                                    )}
                                    ...
                                    <div>
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-white border-2 border-cc-border rounded-xl py-3 pl-12 pr-12 text-cc-text font-bold placeholder:text-cc-muted focus:outline-none focus:bg-cc-yellow/10 focus:shadow-neo transition-all"
                                                required
                                            />
                                    ...
                                    {tab === 'signup' && (
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className={`w-full bg-white border-2 rounded-xl py-3 pl-12 pr-12 text-cc-text font-bold placeholder:text-cc-muted focus:outline-none transition-all ${confirmPass && confirmPass !== password ? 'border-cc-red focus:shadow-neo bg-red-50' : 'border-cc-border focus:bg-cc-yellow/10 focus:shadow-neo'}`}
                                                required
                                            />
                                    ...
                                    {tab === 'login' && (
                                        <div className="flex justify-end mt-1">
                                            <button type="button" className="text-xs font-bold text-cc-muted uppercase tracking-widest hover:text-cc-text transition-colors underline">Forgot password?</button>
                                        </div>
                                    )}
                                    ...
                                    <AnimatePresence>
                                        {error && (
                                            <motion.div
                                                ...
                                                className="text-sm font-bold text-cc-bg bg-cc-red border-2 border-cc-border rounded-xl px-4 py-3 shadow-[2px_2px_0px_#1A1A1A] mt-2"
                                            >
                                                {error}
                                            </motion.div>
                                        )}
                                    ...
                                    >
                                        {loading ? 'Please wait...' : tab === 'login' ? 'Sign In Server' : 'Initialize Account'}
                                    </button>
                                </form>

                                <div className="mt-8 pt-6 border-t-2 border-cc-border/10">
                                    <button
                                        onClick={handleDemo}
                                        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-black uppercase tracking-widest text-cc-text bg-cc-gray hover:bg-cc-yellow border-2 border-cc-border shadow-[2px_2px_0px_#1A1A1A] hover:shadow-[4px_4px_0px_#1A1A1A] hover:-translate-y-0.5 transition-all"
                                    >
                                        <Sparkles size={16} strokeWidth={3} className="text-cc-red" />
                                        Launch Demo App View
                                    </button>
                                </div>
                            </motion.div>
                        </AnimatePresence>
